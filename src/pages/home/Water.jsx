...
export default function Water({ baseYear, compareYear, currentYear, latestDate, isNewMargin, data = {} }) {
  ...
  const values = useMemo(
    () => [...(isNewMargin ? [] : [data.baseYear, data.compareYTM]), data.currentYTM],
    [isNewMargin, data.baseYear, data.compareYTM, data.currentYTM]
  );
  ...
  return (
    <div className="flex h-full w-full items-center justify-around">
      ...
      <div className="flex h-full flex-col items-start justify-center space-y-4 text-lg">
        ...
        <div>{`${t('unit')} : ${t('thousandTon')} / ${t('billionNtd')}`}</div>
      </div>
    </div>
  );
}
